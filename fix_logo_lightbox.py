...
# Center Animated Brand Header (Desktop)
...
# Fix the navigate('/') missing on the desktop header text
text_target = """              <div className="flex flex-col items-center">
                <div 
...
text_replacement = """              <div className="flex flex-col items-center cursor-pointer" onClick={() => navigate('/')}>
                <div 
                  className="text-xl lg:text-2xl font-black uppercase tracking-[0.14em] brand-torch-text select-none group-hover:scale-105 transition-transform duration-300\""""
content = content.replace(text_target, text_replacement)


# Finally add the lightbox modal at the end of the App component (before the final `</div>`)
lightbox_code = """
      {/* Logo Lightbox Modal */}
      {isLogoLightboxOpen && (
...
# Insert right before the last closing `</div>` and `);` of the App return
end_target = "    </div>\n  );\n}"
...
if end_target in content:
    ...
    if end_target_2 in content:
         content = content.replace(end_target_2, lightbox_code + "\n" + end_target_2)
    else:
         content = content[:content.rfind('</div>')] + lightbox_code + content[content.rfind('</div>'):]
