with open('src/components/FootballIntroSplash.tsx', 'w') as f:
    f.write("""import React, { useEffect, useState, useRef } from 'react';
...
export const FootballIntroSplash: React.FC<FootballIntroSplashProps> = ({ onComplete }) => {
  ...
  // We only run the main sequence once on mount
  useEffect(() => {
    ...
    // After 8 seconds, switch to video
    const phaseTimeout = setTimeout(() => {
      ...
      if (videoRef.current) {
        ...
        if (!vDur || isNaN(vDur) || vDur === Infinity) vDur = 5; // fallback 5 seconds
        
        const durationMs = vDur * 1000;
        
        setTimeout(() => setIsFadingOut(true), durationMs - 500);
        setTimeout(() => {
          setIsVisible(false);
  ...
  return (
    ...
    >
      {/* Background Video always rendered, hidden during 'text' phase */}
      <video
        ref={videoRef}
        src="/welcome.mp4?v=6"
        muted
