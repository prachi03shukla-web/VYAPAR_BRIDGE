...
if (postItemIndex !== -1) {
  ...
  code = code.replace(
    /className="w-full h-full max-h-\[80vh\] object-contain bg-black cursor-pointer" \n              onClick=\{onPostClick\}/g,
    \`className="w-full h-full max-h-[80vh] object-contain bg-black cursor-pointer" \n              onClick={handleMediaInteraction}\`
  );

  code = code.replace(
    /className="w-full h-full max-h-\[80vh\] object-contain bg-black transform-gpu will-change-transform cursor-pointer" onClick=\{onPostClick\}/g,
